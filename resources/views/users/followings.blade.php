        ...
        <div class="col-xs-8">
            <ul class="nav nav-tabs nav-justified">
                ...
                <li role="presentation" class="{{ Request::is('users/*/favoritings') ? 'active' : '' }}"><a href="{{ route('users.favoritings', ['id' => $user->id]) }}">Joined events <span class="badge">{{ $count_favoritings }}</span></a></li>
                           <li role="presentation" class="{{ Request::is('users/*/doneings') ? 'active' : '' }}"><a href="{{ route('users.doneings', ['id' => $user->id]) }}">done <span class="badge">{{ $count_doneings }}</span></a></li><!--<li role="presentation" class="{{ Request::is('users/*/favoriters') ? 'active' : '' }}"><a href="{{ route('users.favoriters', ['id' => $user->id]) }}">Favoriters<span class="badge">{{ $count_favoriters }}</span></a></li>-->
            </ul>
